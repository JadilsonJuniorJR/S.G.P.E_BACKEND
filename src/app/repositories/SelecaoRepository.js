...
class SelecaoRepository {
    ...
    create(nome_req, data_inicio, hora_inicio, data_termino, hora_termino, tolerancia, descricao_req) {
        ...
        conexao.query(sql_inserir_evento, [nome_req, data_inicio, hora_inicio, data_termino, hora_termino, tolerancia, descricao_req], (erro, resultado) => {
            if (erro) {
                res.status(404).json({ 'ERRO': 'ERRO AO INSERIR NA TABELA' })
            } else {
                res.status(201).send({
                    status: 'OK',
                    sucesso: true
